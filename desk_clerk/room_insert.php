...
$db=mysql_select_db('workspace',$conn) or die('Database doesnot exist');
			if(isset($_POST['add_cash'])){
				...
				$sql1 = "INSERT INTO shift_report (room_no, amount, type, date, time, user_id) VALUES ('$v_room', '$v_amount', 'Cheque', NOW(), NOW(), '$u_id', NOW(), NOW(), '$u_id')";
				$result1 = mysql_query($sql1,$conn);
				if($result1 == 1)
				echo "<script language='javascript' type='text/javascript'>document.location.href='shift_report.php';</script>";	
				...
				$sql1 = "INSERT INTO shift_report (room_no, amount, type, date, time, user_id) VALUES ('$v_room', '$v_amount', 'Other')";
				$result1 = mysql_query($sql1,$conn);
				if($result1 == 1)
				echo "<script language='javascript' type='text/javascript'>document.location.href='shift_report.php';</script>";	
